...
void SettingsIO::readSettings(Settings &settings) {
	...
	settings.convertRpfToAscii_ = qs_.value(convertRpfToAsciiKey, false).toBool();
	settings.addRpfToMap_ = qs_.value(addRpfToMapKey, false).toBool();
	...
	settings.currentCollectionProfile_ = (Settings::CollectionProfile)qs_.value(collectionProfileKey, 0).toInt();
}
...
void SettingsIO::writeSettings(const Settings &settings) {
	...
	qs_.setValue(convertRpfToAsciiKey, settings.convertRpfToAscii_);
	qs_.setValue(addRpfToMapKey, settings.addRpfToMap_);
	...
	qs_.setValue(collectionProfileKey, settings.currentCollectionProfile_);
}
