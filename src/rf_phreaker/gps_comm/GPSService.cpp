...
void GPSService::initSentenceHandlers(){
	...
	*/
	...
	parser.setCommandHandler("GPGGA", [this](NMEASentence nmea){
		...
		*/
		try
		{
			...
			if (lockupdate){
				this->onLockStateChanged(this->fix.haslock);		//calling handlers
			}
	...
	// Haven't actually seen this as output, but I think it can be configured...
	parser.setCommandHandler("GPVTG", [this](NMEASentence nmea){
		/*
