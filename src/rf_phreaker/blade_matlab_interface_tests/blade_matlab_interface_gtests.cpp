...
TEST(BladeMatlabInterface, TestMain)
{
	...
	EXPECT_EQ(0, list_blade_devices(devices, buf_size));
	...
	int usb_speed;
	EXPECT_EQ(0, get_connected_device_info(device, buf_size, &usb_speed));
	...
	char error[buf_size];
	EXPECT_EQ(0, get_last_error(error, buf_size));
	EXPECT_EQ(0, get_last_error(error, buf_size));
