...
public:
	...
	virtual std::future<scanner*> get_scanner() = 0;

	virtual std::future<rf_phreaker::gps> get_gps_data() = 0;

	virtual std::future<measurement_info> get_rf_data(rf_phreaker::frequency_type freq, int time_ms, rf_phreaker::bandwidth_type bandwidth, rf_phreaker::frequency_type sampling_rate = 0) = 0;
};
