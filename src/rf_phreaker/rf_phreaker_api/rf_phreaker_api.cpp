...
RP_LIBEXPORT(rp_status) rp_get_iq_data_using_auto_gain(rp_device *device, rp_frequency_type freq, rp_time_type time_ns, rp_bandwidth_type bandwidth, rp_frequency_type sampling_rate, rp_raw_data *raw_data) {
	return rf_phreaker_impl::instance().get_iq_data_using_auto_gain(device, freq, time_ns, bandwidth, sampling_rate, raw_data);
}

RP_LIBEXPORT(rp_status) rp_get_iq_data(rp_device *device, rp_frequency_type freq, rp_time_type time_ns, rp_bandwidth_type bandwidth, rp_frequency_type sampling_rate, int32_t gain_db, rp_raw_data *raw_data) {
	return rf_phreaker_impl::instance().get_iq_data(device, freq, time_ns, bandwidth, sampling_rate, gain_db, raw_data);
}
