...
public:
	...
	static double calculate_max_correlation_size(frequency_type freq, double error, double chips_per_second) {
		return calculate_max_coherent_correlation_seconds(freq, error) * chips_per_second; // num chips in 1 second
	}

	static double calculate_max_coherent_slots(frequency_type freq, double error, double slots_per_second) {
		return calculate_max_coherent_correlation_seconds(freq, error) * slots_per_second + 1;
	}

	static double calculate_max_psch_incoherent_iterations(double error, int coherent_slots, double slots_per_second, double chip_rate_hz) {
		return .75 / (error * chip_rate_hz) * slots_per_second; // .75 corresponds to how much of a chip off do we allow
	}
